...
The Atlas since-last-visit panel SHALL ensure the following: On a first visit the panel writes the current Unix-second timestamp to `atlas-lastvisit:<project>` and renders a `delta-first` note that states the number of tracked specs.
...
The Atlas since-last-visit panel SHALL ensure the following: On a return visit the panel lists every spec whose `updated_ts` is strictly greater than the stored timestamp, sorted newest first, each showing the spec module plus optional `updated` and `commits` meta.
...
The Atlas since-last-visit panel SHALL ensure the following: When no spec is newer than the stored timestamp the panel renders a `delta-empty` note that includes the relative time since the last visit.
...
The Atlas since-last-visit panel SHALL ensure the following: The relative time reads as minutes (`m`), hours (`h`), days (`d`), months (`mo`), or years (`y`) using the same thresholds as `rel` in `src/since.js`.
...
The Atlas since-last-visit panel SHALL ensure the following: The current visit timestamp is written only after the delta against the previous timestamp is rendered.
...
The Atlas since-last-visit panel SHALL ensure the following: If `#atlas-data` or `#delta-body` is missing, or the model JSON fails to parse, the panel makes no DOM or storage changes.
