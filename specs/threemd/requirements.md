...
The Atlas 3MD experience SHALL ensure the following: Each `.tmd` viewer renders `model.threemd[data-doc].planes` and starts at
...
The Atlas 3MD experience SHALL ensure the following: The label reads `z=<z> - <label>  (<i+1>/<n>)`, omitting the `z=` prefix when
  a plane's `z` is `'-'`.
...
The Atlas 3MD experience SHALL ensure the following: Plane bodies render only escaped, allow-listed Markdown: headings, `-`/`*`
...
The Atlas 3MD experience SHALL ensure the following: Link hrefs are kept only for relative URLs or `https`/`http`/`mailto`
...
The Atlas 3MD experience SHALL ensure the following: Clicking an `.xlink` shows the plane whose `z` equals `data-z`, or does
...
The Atlas 3MD experience SHALL ensure the following: `copy-stub` output is byte-for-byte identical to `--scaffold` for `clusters[0]`
...
The Atlas 3MD experience SHALL ensure the following: Copy actions use `navigator.clipboard` and fall back to a textarea +
...
The Atlas 3MD experience SHALL ensure the following: `copy-review`/`copy-orphans` flash the row count; `copy-orphans` is sorted by
  LOC descending.
