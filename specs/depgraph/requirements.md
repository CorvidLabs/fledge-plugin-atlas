...
The implementation SHALL satisfy the following criterion: Edges are read from each spec's `depends_on` list, resolved by module name against the model; self references and unresolved names are dropped, and duplicate edges collapse to one.
...
The implementation SHALL satisfy the following criterion: Nodes are placed in levels by the longest dependency chain beneath them; level 0 (depends on nothing) renders at the bottom row.
...
The implementation SHALL satisfy the following criterion: Node radius follows `clamp(11 + sqrt(max(loc,1)) / 3.2, 13, 40)`, so larger specs draw larger discs within the 13 to 40 px bounds.
...
The implementation SHALL satisfy the following criterion: A node is a hub (ringed with `dep-ring` at `r + 4`) when its in-degree is >= 3, or when it is the single most-depended-on node and the max in-degree is >= 2.
...
The implementation SHALL satisfy the following criterion: Cycle-closing edges and their arrowheads render in `var(--bad)`, and the nodes they touch carry the `cyc` class.
...
The implementation SHALL satisfy the following criterion: The viewBox is `-70 0 W+140 H`, reserving 70 px of horizontal slack per side so edge-most node labels are not clipped.
...
The implementation SHALL satisfy the following criterion: With zero edges, the SVG wrapper is hidden and `#deps-note` shows the "No spec declares depends_on..." note (or "No specs found to map." when there are no specs).
...
The implementation SHALL satisfy the following criterion: Hovering a node lights it and its immediate neighbours, marks connected edges/arrows `hot`, and fills `#deps-tip` with LOC, hub/cycle flags, and the depends-on / depended-on-by lists.
...
The implementation SHALL satisfy the following criterion: A summary line in `#deps-note` reports participant count, edge count, hubs, and cycle-edge count.
